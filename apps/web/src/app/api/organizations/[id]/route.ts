...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (parsed.data.parent_organization_id === id) {
      return NextResponse.json({ error: "Les informations saisies sont invalides.", fields: [{ field: "parent_organization_id", message: "Une organisation ne peut pas etre son propre parent." }] }, { status: 400 });
    }
