...
test.describe("Relationships authenticated flow", () => {
  ...
  test("login, create, search, edit, duplicate warning, delete, logout", async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`${personPath.replace(/[.*+?^${}()|[\]\\]/g, "\\    await page.getByRole("button", { name: "Enregistrer" }).click();
    await expect(page.getByText("82")).toBeVisible();

    await page.goto("/relationships/new");")}import { expect, test } from "@playwright/test";
